    ...
    for fst_ball_index in range(len(balls_list[:-1])):
        ...
        for snd_ball_index in inner_range:
            ...
            if (are_colliding_balls(fst_ball, snd_ball)):
                elastic_collision(fst_ball, snd_ball, 1)



def elastic_collision(ball_a, ball_b, e):

    ma = ball_a.get_diameter()
    mb = ball_b.get_diameter()

    ca_x, ca_y = ball_a.get_center()
    cb_x, cb_y = ball_b.get_center()
    Vai_x, Vai_y = ball_a.get_velocity()
    Vbi_x, Vbi_y = ball_b.get_velocity()

    Vaf_x = (Vai_x*(ma-mb) + 2* mb * Vbi_x)/ (ma+mb)
    Vaf_y = (Vai_y*(ma-mb) + 2* mb * Vbi_y)/ (ma+mb)

    Vbf_x = (Vbi_x*(ma-mb) + 2* mb * Vai_x)/ (ma+mb)
    Vbf_y = (Vbi_y*(ma-mb) + 2* mb * Vai_y)/ (ma+mb)

    ball_a.set_velocity(Vaf_x , Vaf_y)
    ball_b.set_velocity(Vbf_x,  Vbf_y)
